...
*/
function expandInfo(obj: Record<string, Record<string, string>>): Record<string, Record<string, string>> {
  Object.keys(obj).forEach(key => {
    obj[key] = expandCsvKeyObject(obj[key]);
  });
  return obj;
}
...
// Find info for a given config key. If no fileType, will match config keys for 'any' type.
export function matchConfigKeyInfo(key: string, fileType: string): string | undefined {
  const endingNums = key.match(/\d+$/);
  if (endingNums) {
    key = key.substring(0, key.indexOf(endingNums[0]));
  }
