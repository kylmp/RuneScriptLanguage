...
const enumPostProcessor = function(identifier) {
  ...
  identifier.inputTypeKeyword = inputtypeLine.substring(10);
  identifier.outputTypeKeyword = outputtypeLine.substring(11);
}
...
const paramPostProcessor = function(identifier) {
  ...
  identifier.dataTypeKeyword = typeLine.substring(5);
}
...
const triggerPostProcessor = function(identifier) {
  const info = matchTriggerInfo(identifier.name);
  info ? identifier.info = info : identifier.hideDisplay = true;
}

module.exports = { queuePostProcessor, coordPostProcessor, enumPostProcessor, paramPostProcessor, configKeyPostProcessor, triggerPostProcessor };
