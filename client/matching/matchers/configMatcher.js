...
async function configMatcher(context) {
  ...
  if (context.prevChar === '[' && context.nextChar === ']') {
  ...
  if (CONFIG_LINE.test(context.line)) {
    ...
    if (context.word.index === 0) {
      const configKeyInfo = matchConfigKeyInfo(configKey, context.fileType);
      return (configKeyInfo) ? reference(matchType.CONFIG_KEY) : matchType.UNKNOWN;
    }
...
async function paramSpecialCase(context) {
  ...
  if (context.word.index === 2) {
    ...
    return matchType[keywordToMatchId(paramIdentifier.dataTypeKeyword)];
  }
...
function valSpecialCase(context) {
  ...
  // data types IDs are defined on the identifier as: identifier.inputTypeKeyword, identifier.outputTypeKeyword
  return matchType.UNKNOWN; 
}
